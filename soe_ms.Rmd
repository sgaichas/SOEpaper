...
Below we give two illustrative examples of SOE reporting from the main body of the report illustrating the priorities for addressing "so what." In the first, we demonstrate how reporting of the same indicator suite was tailored for a single ecological production unit (Mid-Atlantic) and for two ecological production units within one management jurisdiction (New England). This was the first in-depth indicator discussion in each report following the executive summary. In the second example, we demonstrate synthesis across multiple indicators within a report section using the physical oceaographic and low trophic level indicators connecting to managed species in the Mid-Atlantic. 
...
guildlandGOM <- capture_base_plot({
  ...
  soe.plot(SOE.data.2018, "Time","Apex Predator Landings GOM", 
           stacked = "Apex predators",status = F,
...
guildlandGB <- capture_base_plot({
  ...
  soe.plot(SOE.data.2018, "Time","Apex Predator Landings GB", stacked = "A",status = F,
           endshade = T, rel.y.num = 0.6, full.trend = T, scale.axis = 10^3,end.start = 2007, x.start = 1964,
...
## Ecosystem-wide and managed species total landings
totland <- capture_base_plot({
  opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(3, 3.5, 1, 1))
  
  soe.plot(SOE.data.2018, "Time", "Total Landings GOM", point.cex = 1, 
           stacked = "GOM",status = F,
           endshade = T, rel.y.num = 0.6, full.trend = T, scale.axis = 10^3,
           end.start = 2007, x.start = 1964,
           ymin = FALSE, y.lower = 0 , extra = TRUE, x.var2 = "Time", 
...
```{r longtermsst, fig.cap="Long-term sea surface temperatures on the Northeast Continental Shelf",  echo=F, message=FALSE, warning=FALSE , fig.height=2.5, fig.align = 'center',fig.pos='H'}
opar <- par(mar = c(4, 6, 2, 6))
...
```{r SSTPPDCHLMAB, fig.cap="SST (A), primary production (B), and Chl a (C) over 2017 (colored polygons) compared against long-term mean (black line) and +/- 1 standard deviation (grey polygon) in the Mid-Atlantic",echo = F, fig.show='hold', fig.align='default',warning = F, message = F,fig.pos='H',fig.height = 2.75,fig.width = 8}
## SST
