...
The main body of the report follows the executive summary with more detail on the indicator results. The full set of report sections and includes indicators of fishery performance, human community vulnerability, fishery interactions with protected species, resource species trends, and ecosystem consitions and productivity (Table \@ref(SOEcontents)). Many indicators aggregate species by similar functional groups (Table \@ref(tab:guilds)). Below we give illustrative examples of SOE reporting from the main body of the report. In the first, we demonstrate how reporting of the same indicator suite was tailored for a single ecological production unit (Mid-Atlantic) and for two ecological production units within one management jurisdiction (New England). This was the first in-depth indicator discussion in each report following the exectuive summary.
...
```{r objcatsMAB, echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
tabl <- "
...
```{r speciesgroupingsMAB, echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
tabl <- "
...
```{r proplandingsrevMAB, echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
tabl <- "
...
```{r proptableNE,echo = F, message=FALSE, warnings=FALSE, results = 'asis'}
