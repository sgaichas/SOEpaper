...
# Discussion
* Rising to the "so what" challenge improved ecosystem reporting in the NE US and has resulted in other analyses of use to managers (e.g. ref risk assessment, hopefully published by then). An effective annual process has evolved where contextual information is presented that can change multiple decisions if necessary (@zador_ecosystem_2016, value of qualitative information). Flexibility and responsiveness to audience important, willing to change reporting as manager needs change. Retain elements of DPSIR and ecosystem services approaches [refs] while adjusting to regional needs.
...
* Put best practices/lessons learned from IEA group WGNARS [@levin_integrated_2009, @levin_guidance_2014, @depiper_operationalizing_2017] into actual practice. 
...
What should managers do with this information? In the absence of management decision criteria based on indicator thresholds (work is ongoing [refs]), information from these reports has been used in management.
* MAFMC wants to see the SOE report and indicators inform their risk assessment as part of EAFM, want to add management oriented indicators such as other ocean uses and regulatory complexity.
* More stock specific reports in development to go to assessment working groups, see ECSA
...
```{r SSTPPDCHLMAB, fig.cap="SST (A), primary production (B), and Chl a (C) over 2017 (colored polygons) compared against long-term mean (black line) and +/- 1 standard deviation (grey polygon) in the Mid-Atlantic",echo = F, fig.show='hold', fig.align='default',warning = F, message = F,fig.height = 2.75, fig.width = 8, out.width='\\linewidth'}
## SST
...
```{r MABecoprod, fig.cap="Small-large copepod index with primary productivity anomaly (A), \\textit{C. typicus} abundance anomaly (B), groundfish condition (C), and groundfish productivity for MAFMC managed species in the Mid-Atlantic.", echo = F, fig.keep='last', fig.align='center', warning = F, message = F, out.width='\\linewidth'}
