...
```{r, fig.cap="Mid-Atlantic Ecosystem \\label{conceptual}", fig.height=6, fig.width=4, out.width='\\linewidth', fig.align='center', echo = F, message = FALSE, warning = FALSE, eval = T,fig.pos='H'}
...
```{r guild_landings_MAB, fig.cap="MAFMC seafood specific landings (red) and total commericial landings (black). A: Apex predators, B: Piscivore, C: Planktivore, D: Benthivore, E: Benthos \\label{landings}",echo = F, warning = F, include = T, fig.asp=0.78, fig.align='center',fig.pos='H', out.width='0.7\\linewidth'}

## Ecosystem-wide and managed species total landings
opar <- par(mfrow = c(5, 1), mar = c(0, 0, 0, 0), oma = c(4, 6.5, 2, 6))

soe.plot(SOE.data.2018, "Time","Apex Predator Landings MAB", stacked = "Apex Predator",status = F,
...
```{r managed_landings_MAB, fig.cap="Total commercial seafood landings (black), MAFMC managed seafood landings (red)", echo=F, message=FALSE, warning=FALSE , fig.asp=0.35, fig.align = 'center',fig.pos='H'}
...
```{r rec_catch_MAB, fig.cap="Total recreational harvest  \\label{recfood}", echo = F, message=F, include=T, warning=F, fig.asp=0.35, fig.align = 'center',fig.pos='H'}
...
```{r managed_revenue , fig.cap="Total revenue by region and MAFMC managed species\\label{totcommrev}", echo = F, message=F, include=T, warning=F ,  fig.asp=0.35, fig.align = 'center',fig.pos='H'}
opar <- par(mar = c(4, 6, 2, 6))
...
soe.plot(SOE.data.2018,"Time", "Mid-Atlantic fleet count", stacked = "B",
         rel.y.num = 0.9, end.start = 2008, full.trend = F)
...
soe.plot(SOE.data.2018, 'Time', "Mid-Atlantic commercial species diversity", 
         ...
         rel.y.text = 1, rel.y.num = 1, y.line = 2.5, x.line = 2, full.trend = F)
```
...
soe.plot(SOE.data.2018, "Time", "Mid-Atlantic Rec participation", stacked = "A",
         rel.y.num = 0.9, scale.axis = 10^6, end.start = 2007, tol = 0.15)
...
soe.stacked.axis('Year', expression('Recreational Participation, 10'^6*'n'), 
                 rel.x.text = 1, rel.y.text = 1, y.line = 2.5)
```
...
mtext(2, text = expression("Oysters Harvested, 10"^6*'n'), line = 2.5)
  axis(2, at = c(1*10^6,5*10^6,10*10^6,15*10^6,
       ...
       labels = c("1","5","10","15","20",
                  "25","30","35","40"),
       las = 1, cex.axis = 1)
legend(2006, 40000000, legend = c("New Jersey",
                            "Maryland",
                            #"Rhode Island",
...
```{r hab_MAB, fig.cap="Occurrence of all blooms at concentrations warranting action by the Virginia Department of Health (black), and occurrence of \\textit{C. polykrikoides} in Chesapeake Bay at cell concentrations >300,000 cells per L (red).", echo = F, warning = F, include = T,fig.asp=0.5, fig.align='center',fig.pos='H'}
opar <- par(mar = c(4, 6, 2, 6))

soe.plot(SOE.data.2018, "Time", "HAB events at VDH threshold", mean_line = F, rel.y.num = 1.1, 
         full.trend = F, end.start = NA, ymin = FALSE, y.lower = 0, extra = TRUE, 
         x.var2 = "Time", y.var2 = "Cochlodinium polykrikoides at 3e5 thresh", mean_line.2 = F,
...
```{r 'commercial_reliance_and_vulnerability MAB' , fig.cap="Commercial (A) and recreational (B) reliance and social vulnerability (MAB) \\label{commvulmap}",echo = F, message=F, include=T, warning=F, fig.show='hold', fig.width = 8,fig.height = 5.4,strip.white=T,fig.pos='H'}
...
```{r harbor_porpoise, fig.cap="Harbor porpoise bycatch estimated compared with PBR \\label{harbporp}", echo = F, message=F, include=T, warning=F, fig.asp=0.5, fig.align = 'center',fig.pos='H', out.width='0.5\\linewidth'}
opar <- par(mar = c(4, 6, 2, 6))
...
```{r rw_abundance, fig.cap="Right whale population estimate\\label{rightwh}",echo = F, message=F, include=T, warning=F, fig.asp=0.5, fig.align = 'center', fig.pos='H', out.width='0.5\\linewidth'}
opar <- par(mar = c(4, 6, 2, 6))
...
soe.plot(SOE.data.2018, "Time","Fall_Ich_Shannon Diversity Index", stacked = "B",
         rel.y.num = 1.1, full.trend = F, end.start = 2006)
soe.stacked.axis('Year', 'Shannon Index')
...
```{r southern kingfish, fig.cap="Southern kingfish occurrences in the observer database", echo=F,message=FALSE, warning=FALSE ,fig.align='center', fig.asp=.95,fig.width=6,fig.pos='H'}
if(map_figs){
...
```{r chl_ppd_maps,fig.cap="Primary production and chlorophyll anomaly maps \\label{ppchlanom}" ,  fig.width = 8,fig.height = 4, fig.show='hold',fig.pos='H'}
#read images
#read images
